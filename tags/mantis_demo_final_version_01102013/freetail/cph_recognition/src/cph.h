...
public:
  ...
  CPHEstimation2(int num_y, int num_c){
   num_ybins = num_y;
   num_cbins = num_c;
   hist.resize(num_ybins * num_cbins,0);
   centroid.resize(3,0);
...
int compute(std::vector<float> &result){
  ...
  int y,c;
  float dy = y_size/num_ybins;
  float dc = PI/num_cbins;
  //std::cout << "dy: " << dy << "  dc: " << dc << std::endl;
  
  for(unsigned int i=0; i<cloud->size(); i++){
    //bin z component
    y = floor((cloud->points.at(i).y-y_min)/dy);
    double angle = atan2(cloud->points.at(i).z-centroid.at(2),cloud->points.at(i).x-centroid.at(0));
    if(angle < 0){
      c = floor((PI+angle)/dc);
    }else{
       c = floor((angle)/dc); 
