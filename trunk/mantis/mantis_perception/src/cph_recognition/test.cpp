...
int main(int argc, char **argv)
{
  ...
  segmentation_client = n.serviceClient<tabletop_object_detector::TabletopSegmentation>("/mantis_segmentation", true);

  seg_pub = n.advertise<sensor_msgs::PointCloud2>("/segmentation_result",1);

  while (ros::ok())
  {
    mantis_perception::MantisSegmentation seg_srv;
